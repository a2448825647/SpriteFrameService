...
移植自 SpriteFrameStudio 的 src/utils/crossfade.py。
"""
...
def apply_transition_to_frame_data(
...
def apply_loop_crossfade(
    ...
    """重叠交叉淡入淡出：将末尾T帧与开头T帧一一对应混合。输出序列长度 = N - T。"""
    if not frames:
        return []
...
def apply_loop_align(
    ...
    for k in range(T):
        ...
        if alpha == 0.0:
            ...
            result.append(_align_interpolate(tail_frame, head_frame, alpha))
...
def _align_interpolate(src: np.ndarray, dst: np.ndarray, alpha: float) -> np.ndarray:
    ...
    shifted_src = _shift_image(src, cx_target - cx_src, cy_target - cy_src)
    shifted_dst = _shift_image(dst, cx_target - cx_dst, cy_target - cy_dst)
...
def _ensure_same_channels(img_a: np.ndarray, img_b: np.ndarray,
                          ch_a: int, ch_b: int) -> Tuple[np.ndarray, np.ndarray]:
    """确保两个图像通道数一致。"""
...
def _match_channels(img_a: np.ndarray, img_b: np.ndarray,
                    channels_a: int, channels_b: int) -> Tuple[np.ndarray, np.ndarray, bool]:
